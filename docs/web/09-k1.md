...
~~~
### 服务端 Canvas 渲染
如果你希望输出的是一张图片而非 SVG 字符串，或者你还在使用更老的版本，我们会推荐使用 node-canvas 来实现 ECharts 的服务渲染，node-canvas 是在 NodeJS 上的一套 Canvas 实现，它提供了跟浏览器中 Canvas 几乎一致的接口。
...
#### 图片的加载
node-canvas 提供了图片加载的`Image`实现，如果你在图表中使用了到了图片，我们可以使用5.3.0新增的`setPlatformAPI`接口来适配。
~~~
